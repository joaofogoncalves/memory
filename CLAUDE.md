...
│   │   ├── logo.svg                  # JG monogram (path data inlined into build.py's nav)
│   │   └── favicons                  # favicon.ico, favicon-192.png, apple-touch-icon.png
│   └── dist/                         # Generated site output (git-ignored)
│
├── charts/                           # Chart/diagram renderer (Node + Playwright + Chart.js)
│   ├── render.mjs                    # CLI: JSON + template → .webp/.png (run from repo root)
│   ├── theme.js                      # Shared palette/typography (dark navy + teal)
│   ├── templates/                    # Standalone HTML templates (bar, flow, quadrant, quote-card, …)
│   ├── examples/                     # Sample JSON payloads per template
│   ├── package.json + lock           # npm project ("charts") — playwright, sharp, yargs
...
│
├── content/                          # Personal content you author & own (tracked)
│   ├── cv.md                         # Professional CV + About-page narrative sections
│   ├── cv_joaofogoncalves.pdf        # Generated CV PDF (tracked asset; build copies to dist/about/)
│   ├── now.md                        # /now page content
│   ├── now.md.example                # /now template
│   ├── work.md                       # /work page content (case studies)
│   ├── pitches.md                    # /pitch output: living doc with all pitch variants
│   ├── posts/                        # Short-form posts (authored via /post + historical scrape)
│   │   └── YYYY/MM/DD-slug/          # post.md (+ linkedin.md, x-thread.md) + media/
│   └── articles/                     # Long-form articles
│       └── YYYY/MM/DD-slug/          # article.md, image-prompts.md, media/
│
├── style/                            # Voice/visual guides feeding the writing skills (tracked)
│   ├── writing_style.md              # LinkedIn writing style guide (primary for /post)
│   ├── article_style.md              # Long-form article style supplement
│   ├── pitch_style.md                # Self-positioning guide (/cover-letter, /outreach, /pitch, About)
│   ├── profile.md                    # Voice profile (generated by /profile)
│   └── taste.md                      # Visual taste profile (generated by /taste)
│
├── cv/                               # CV PDF generator (self-contained)
│   ├── generate.py                   # reportlab-based PDF builder (reads content/cv.md)
...
│   └── stitch-prompt.md              # Original website design brief
│
...
- Structure: `content/posts/YYYY/MM/DD-slug/`
...
- **Site is canonical for short-form posts.** Authored via `/post`, which saves `post.md` (canonical body) plus `linkedin.md` and `x-thread.md` (paste-ready variants for manual posting). `content/posts/` is tracked in git — these are authored artifacts the user owns, not just a scraped mirror.
- The historical `content/posts/` archive (scraped from LinkedIn before this model existed) lives alongside newly-authored posts; both are rendered the same way by the site builder.
...
### 11. CV PDF Generator
- `cv/generate.py` — self-contained reportlab script that generates `content/cv_joaofogoncalves.pdf` from `content/cv.md`
...
- `content/cv_joaofogoncalves.pdf` is tracked in git as a persistent asset; `build.py` copies it to `dist/about/` during build
- **Why:** Native PDF generation avoids Playwright/browser dependency and is faster and more portable

### 12. Writing Style Guide
- `style/writing_style.md` — authoritative style guide for LinkedIn post writing
- Defines structure, tone, language rules, length targets, and anti-patterns
- The `/post` skill uses it as primary reference (takes precedence over `style/profile.md` where they conflict)
- `style/profile.md` supplements with vocabulary, topic expertise, and deeper voice patterns

### 13. Articles Section
- `content/articles/` directory stores original long-form content (tracked in git, same as `content/posts/` now)
- Structure: `content/articles/YYYY/MM/DD-slug/article.md` with `media/` subdirectory (directory name is the zero-padded day + slug; `build.py` derives year/month/date from frontmatter and uses the directory name as the URL slug, so the date isn't repeated in the name)
...
- To promote a draft: run `/publish <slug>` — removes `draft: true`, updates `date:` to today, and renames the directory to match the new date so the path stays `content/articles/YYYY/MM/DD-slug/`.
- `style/article_style.md` — supplements `style/writing_style.md` with long-form patterns (section headers, citations, pacing)
...
|---|---|---|---|
| Personal narrative (always-on) | n/a (manual) | About page (rendered from `content/cv.md` Hero/Thesis/Building/Open To sections) | Replaces conventional bio + timeline. Numbered sections, metric callouts, soft pitch at bottom. |
| Generic CV (always-on) | `/pdf` (no args) | `content/cv_joaofogoncalves.pdf` | Reads `content/cv.md` as source of truth; tracked in git. |
| Self-pitch variants (always-on) | `/pitch` (optional variant arg) | `content/pitches.md` (tracked) | 9 variants in one file: 30s/60s/2min elevator, LinkedIn headline + About, talk abstract, 3 speaker bios. |
...
`content/cv.md` is the source of truth for everything. Its sections split into two audiences:

- **CV-only sections** (`Summary`, `Experience`, `Education`, `Languages`, `Top Skills`, `Certifications`) — render in `content/cv_joaofogoncalves.pdf` and `applications/{slug}/cv.pdf`.
...
- `style/pitch_style.md` — primary authority for any artifact where João is the subject (About page, cover letters, outreach, interview prep, pitches). Codifies the four principles above plus length targets per surface and an anti-pattern checklist.
- `style/writing_style.md` — for LinkedIn posts (still primary for `/post`)
- `style/article_style.md` — for long-form essays (still primary for `/article`)
...
### 16. Chart Renderer (`charts/`)
- `charts/render.mjs` — a Node + Playwright + Chart.js CLI that renders JSON specs into on-brand `.webp`/`.png` visuals from the standalone HTML templates in `charts/templates/`
- Invoked from the repo root: `node charts/render.mjs --template <name> --data <spec>.json --output <file>`. The `/post` and `/article` skills generate the JSON spec and call it; rendered files are saved into the relevant post/article `media/` dir
- Self-contained npm project: `charts/package.json` + `charts/node_modules/` (deps: `playwright`, `sharp`, `yargs`). First-time setup, run from `charts/`: `npm install && npx playwright install chromium`
- Templates: `bar`, `stat-compare`, `quadrant`, `line`, `feature-compare`, `grid-diff`, `flow`, `timeline`, `article-card`, `quote-card`, `quote-classic`. Shared dark-navy + teal theme in `charts/theme.js`; per-template data shapes in `charts/README.md`
- Output to `charts/out/` (git-ignored)
- **Why:** charts render as crisp, consistent, on-brand visuals — they outperform generic AI illustrations for any structural or data-driven argument
...
bash scripts/pipeline.sh --skip-scrape
...
```yaml
output:
  base_dir: content/posts  # Relative to project root, or absolute path
...
**Pipeline (required for `bash scripts/pipeline.sh`):**
```env
...
- Saves to `content/articles/YYYY/MM/DD-slug/article.md`
...
- Renames the directory (`content/articles/YYYY/MM/DD-slug/`) via `git mv` so the new date is reflected in the path
...
- Writes four artifacts under `content/posts/YYYY/MM/DD-slug/`:
...
- Run before `scripts/pipeline.sh --skip-scrape` to keep engagement data fresh and catch stragglers
...
- Reads recent `content/articles/*` for thesis material to surface in answers
...
- Saves to `content/pitches.md` (single living document with all variants under headers)
...
**Site build fails:**
- Ensure posts exist in `content/posts/` directory
...
- Documentation (`README.md`, `CLAUDE.md`, and the `docs/` folder: `CONTRIBUTING.md`, `RATE_LIMITS.md`, `stitch-prompt.md`)
- Configuration templates (`config/config.yaml`, `config/site.yaml.example`)
- Example files (`content/now.md.example`)
...
- `content/` — personal content the user owns: `cv.md`, `now.md`, `work.md`, `pitches.md`, plus the generated `cv_joaofogoncalves.pdf` (persistent asset)
- `style/` — voice/visual guides feeding the skills: `writing_style.md`, `article_style.md`, `pitch_style.md`, `profile.md`, `taste.md`
- `charts/` — chart renderer source (`render.mjs`, `theme.js`, `templates/`, `examples/`, `package.json`, `package-lock.json`)
- `content/articles/` (original long-form articles — authored content, not scraped)
- `content/posts/` (short-form posts — authored via `/post` and historical scrape; the authored-first model treats these as canonical site content the user owns)
...
- `.venv/`, `cache/`, `logs/`, `web/dist/`, `drafts/`, `node_modules/` (incl. `charts/node_modules/`), `charts/out/`
...
3. Verify output in `content/posts/` directory
...
1. Identify which doc needs update (README, the `docs/` folder, or this file)
...
- Reads markdown posts from `content/posts/`, articles from `content/articles/`, About content from `content/cv.md`, Now content from `content/now.md`
...
- `parse_all_articles()` — scans `content/articles/` for `article.md` files, extracts frontmatter (title, subtitle, hero_image, substack_url, reading_time)
...
### charts/render.mjs
- Node + Playwright + Chart.js CLI that renders JSON specs into `.webp`/`.png` visuals from the HTML templates in `charts/templates/`
- Run from the repo root: `node charts/render.mjs --template <name> --data <spec>.json --output <file>`. Self-contained npm project (`charts/package.json`, `charts/node_modules/`); first-time setup runs `npm install && npx playwright install chromium` from `charts/`
- Shared dark-navy + teal theme in `charts/theme.js`; see `charts/README.md` for templates and per-template data shapes
- Output to `charts/out/` (git-ignored)

### cv/generate.py
- Self-contained reportlab script that generates `content/cv_joaofogoncalves.pdf` from `content/cv.md`
...
- Run: `uv run python cv/generate.py` → outputs `content/cv_joaofogoncalves.pdf`
...
- docs/RATE_LIMITS.md - API rate limit details
