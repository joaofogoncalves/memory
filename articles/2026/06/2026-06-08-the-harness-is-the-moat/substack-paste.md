# Substack paste-in — The Harness Is the Moat

## Posting checklist (do these in Substack's editor)

1. **Title:** The Harness Is the Moat
2. **Subtitle:** The model is the part everyone benchmarks. The reliability layer around it is the part that actually ships.
3. **Canonical URL** (Post settings → SEO → Canonical URL): `https://joaofogoncalves.com/articles/2026/06/2026-06-08-the-harness-is-the-moat/` — keeps SEO pointed at your site, not Substack.
4. **Hero image:** upload `media/hero.jpg` at the top of the post (Substack strips local paths on paste; upload through their UI).
5. **Inline images:** re-upload each as you reach it in the body (they appear as `[IMAGE: ...]` placeholders):
   - `media/model-one-stage.webp`
   - `media/harness-depreciation.webp`
   - `media/rented-vs-owned.webp`
6. **Tags:** ai, software-engineering, agents, claude-code, engineering-leadership
7. At the end of the post, add this canonical-pointer line:

   > Originally published at [joaofogoncalves.com/articles/2026/06/the-harness-is-the-moat](https://joaofogoncalves.com/articles/2026/06/2026-06-08-the-harness-is-the-moat/).
...
[IMAGE: A single feature's path through the harness. The model is one highlighted stage among many; the gates and recovery steps around it are the harness. — upload media/model-one-stage.webp here]
...
[IMAGE: Two lines cross: as model capability rises across Opus 4.5, 4.6, and 4.8, the scaffolding the model still needs falls away. The harness encodes the model's current weaknesses, and they expire on each release. — upload media/harness-depreciation.webp here]
...
[IMAGE: Two columns. Left, muted: the rented half of the harness, the generic agent loop, sandboxes, and session state that Anthropic now sells as a hosted service. Right, in teal: the owned half, your deploy gates, domain verification, accumulated failure history, and the skill of rebuilding it when the model moves. The moat is the right column. — upload media/rented-vs-owned.webp here]
...
- Optional: share the Substack URL in a Notes post for an extra discovery pass.
