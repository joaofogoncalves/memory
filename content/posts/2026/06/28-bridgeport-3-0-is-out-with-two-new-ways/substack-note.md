# Substack Note — bridgeport-3-0-is-out-with-two-new-ways

Paste into Substack Notes (substack.com/notes). No hashtags. Link welcome inline.

---

BridgePort 3.0 is out, with two new ways to drive it: a Terraform provider, so your config lives in version control and drift shows up in terraform plan, and an MCP server, so you can operate it from an AI agent in your editor.
...
Full write-up, how the two surfaces are the same API contract projected two ways:
