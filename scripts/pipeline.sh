...
#
# Usage:
#   bash scripts/pipeline.sh                # scrape + resolve + deploy
#   bash scripts/pipeline.sh --skip-scrape  # resolve + deploy only (skip crawl)
#   bash scripts/pipeline.sh --dry-run      # scrape + resolve (no deploy)
set -euo pipefail

# This script lives in scripts/; resolve the repo root one level up.
...
if [ "$SKIP_SCRAPE" = false ]; then
  ...
  uv run python "$ROOT_DIR/scraper/main.py" --crawl --profile-url "$LINKEDIN_PROFILE_URL" $LIMIT_FLAG
  echo ""
fi

# ── 2. Resolve shortened URLs ────────────────────────────────────────────────
echo "=== Resolving shortened URLs ==="
uv run python "$ROOT_DIR/web/resolve_links.py"
echo ""

# ── 3. Build & deploy ────────────────────────────────────────────────────────
if [ "$DRY_RUN" = false ]; then
  echo "=== Building & deploying site ==="
  bash "$ROOT_DIR/web/deploy.sh"
else
  echo "=== Dry run: building site only (no deploy) ==="
  uv run python "$ROOT_DIR/web/build.py"
fi
